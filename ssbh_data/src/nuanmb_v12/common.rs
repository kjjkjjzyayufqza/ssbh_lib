...
pub(super) const G_CURVE_NIBBLE_SCALE: f32 = 0.142_857_15;
    ...
    fn generate() -> Self {
        ...
        for v18 in 1..=8 {
            ...
            for row in 0..dim {
                for col in 0..dim {
                    let val =
                        ((col as f32 + 0.5) * ((row as f32 + 0.5) * (PI / dim as f32))).cos()
                            * scale;
                    data.push(val);
                }
            }
...
pub(super) fn try_pick_comp_bits_4309(
    ...
    for comp_bits in [4, 3, 2, 1] {
        let q_counts =
            match compute_block_qcounts_4309(payload, residual_start0, base_scale, comp_bits, key_count) {
                Ok(q) => q,
                Err(_) => continue,
            };
...
pub(super) fn quat_normalize(x: f32, y: f32, z: f32, w: f32) -> Vector4 {
...
pub(super) fn quat_nlerp(q0: Vector4, q1: Vector4, t: f32) -> Vector4 {
...
pub(super) fn quat_dot(a: Vector4, b: Vector4) -> f32 {
    a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w
}

pub(super) fn quat_neg(q: Vector4) -> Vector4 {
    Vector4 {
        x: -q.x,
        y: -q.y,
        z: -q.z,
...
pub(super) fn reconstruct_quat_from_xyz(x: f32, y: f32, z: f32, sign: f32) -> Vector4 {
    let s = x * x + y * y + z * z;
