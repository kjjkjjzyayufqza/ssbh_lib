use super::common::{
    ...
    compute_block_len_type9, compute_block_qcounts, decode_residual_vector, expand_sparse_vec3,
    read_f32_le, read_u16_le, read_u32_le, read_vec3_f32_le, G_CURVE_SHORT4_SCALE,
};
...
pub fn decode_translate_3200(bytes: &[u8]) -> Result<Vec<Vector3>, error::Error> {
    ...
    Ok(expand_sparse_vec3(
        &frame_indices,
        &values.iter().map(|v| (v.x, v.y, v.z)).collect::<Vec<_>>(),
        total_frames,
    ))
}
