...
/// Uses format 0x1013 for constant values or 0x1019 stream for multi-frame data.
pub fn create_v12_compressed_bool_data(values: &[bool]) -> Result<Vec<u8>, Error> {
    // Compression for booleans currently falls back to the same layouts as the
    // uncompressed path; both are understood by `read_property_value_v12`.
    create_v12_uncompressed_bool_data(values)
}
