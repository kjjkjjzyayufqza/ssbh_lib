use crate::anim_data::{
    AnimData, GroupData, NodeData, TrackData, TrackValues, error::Error,
    v2::buffers::read_track_values_v2,
};
use binrw::io::{Cursor, Seek, Write};
use ssbh_lib::{
    SsbhArray,
    formats::anim::{Anim, CompressionType, Group, Node, TrackFlags, TrackV2, UnkData},
};

mod buffers;
mod compression;

pub fn create_anim_v20(data: &AnimData) -> Result<Anim, Error> {
...
pub fn create_anim_v21(data: &AnimData) -> Result<Anim, Error> {
    let mut buffer = Cursor::new(Vec::new());
    let animations = create_anim_groups_v2(data, &mut buffer)?;
...
fn final_frame_index(data: &AnimData, max_frame_count: u32) -> Result<f32, Error> {
    if data.final_frame_index >= 0.0 && data.final_frame_index >= max_frame_count as f32 - 1.0 {
        Ok(data.final_frame_index)
    } else {
        Err(Error::InvalidFinalFrameIndex {
            final_frame_index: data.final_frame_index,
        })
    }
}

fn create_anim_group_v2(g: &GroupData, buffer: &mut Cursor<Vec<u8>>) -> Result<Group, Error> {
    Ok(Group {
...
fn create_anim_node(n: &NodeData, buffer: &mut Cursor<Vec<u8>>) -> Result<Node, Error> {
    Ok(Node {
...
fn create_anim_track_v2(buffer: &mut Cursor<Vec<u8>>, t: &TrackData) -> Result<TrackV2, Error> {
...
pub fn read_groups_v20(
    anim_groups: &[ssbh_lib::formats::anim::Group],
    anim_buffer: &[u8],
) -> Result<Vec<GroupData>, Error> {
...
pub fn create_track_data_v20(
    ...
    let end = start
        ...
        .ok_or(Error::InvalidTrackDataRange {
            start: track.data_offset as usize,
            size: track.data_size as usize,
            buffer_size: buffer.len(),
        })?;
    let buffer = buffer.get(start..end).ok_or(Error::InvalidTrackDataRange {
        start: track.data_offset as usize,
        size: track.data_size as usize,
        buffer_size: buffer.len(),
    })?;
...
mod tests {
    use glam::Vec4;
    use ssbh_lib::formats::anim::{TrackTypeV2, TransformFlags as AnimTransformFlags};

    use super::*;

    use crate::anim_data::{Transform, TransformFlags, UvTransform};
    ...
    fn read_v20_track_invalid_offset() {
        ...
        assert!(matches!(
            result,
            Err(Error::InvalidTrackDataRange {
                start: 5,
                size: 1,
                buffer_size: 4
            })
